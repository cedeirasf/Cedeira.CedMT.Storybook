...
export const CBSTProvider = ({ children, parentHeight }: CBSTProviderProps) => {
  ...
  const setTableSizes = useState<ITableSizeNoGroup>({ column: [], rows: [] })[1]
  ...
  const handleScroll = (event: React.UIEvent<HTMLDivElement>, sourceIndex: number) => {
  ...
  return (
    <div ref={containerRef} className="h-full">
      <CBSTContext.Provider value={contextValue}>{children}</CBSTContext.Provider>
    </div>
  )
}
