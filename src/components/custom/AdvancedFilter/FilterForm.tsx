...
import { DataType, FilterFormData, FilterFormProps, RangeValue } from "@/types/components/custom-advanced-input-filter.type"
...
export function FilterForm({ initialFilter, onSubmit, filterScheme, sources }: FilterFormProps) {
  ...
  if (isGenericFilter) {
    return <GenericFilterForm control={control} errors={errors} handleSubmit={handleSubmit} />
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <FormField label="Valor" name="value" errors={errors}>
        <ValueInput control={control} dataType={getDataTypeForSelectedField()} operator={selectedOperator} />
      </FormField>
...
const FormField: React.FC<FormFieldProps> = ({ label, name, errors, children }) => (
  <div className="space-y-2">
    <Label htmlFor={name}>{label}</Label>
    {children}
    {errors[name as keyof FilterFormData] && <p className="text-sm text-destructive">{errors[name as keyof FilterFormData]?.message}</p>}
  </div>
)
...
const GenericFilterForm: React.FC<GenericFilterFormProps> = ({ control, errors, handleSubmit }) => (
  <form onSubmit={handleSubmit} className="space-y-4">
    <FormField label="Valor" name="value" errors={errors}>
      <Controller
        name="value"
        ...
        <Popover>
          <PopoverTrigger asChild>
            ...
            >
              ...
              {currentValue ? `: ${format(new Date(convertToISODate(currentValue as string)), "dd-MM-yyyy")}` : ""}
            </Button>
          </PopoverTrigger>
...
function renderSingleInput(dataType: DataType, control: Control<FilterFormData>) {
  return (
    <Controller
      name="value"
      control={control}
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    Seleccionar fecha
                    {field.value ? `: ${format(new Date(convertToISODate(field.value as string)), "dd-MM-yyyy")}` : ""}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start">
                  <CustomCalendar
