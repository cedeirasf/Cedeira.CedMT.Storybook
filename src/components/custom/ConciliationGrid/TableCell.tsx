...
const hasBgStyle = (styles?: readonly string[]): boolean => {
  return styles?.some((style) => style.includes("bg-") || style.includes("background")) ?? false
}
...
export const TableCell: React.FC<TableCellProps> = memo(({ columnKey, column, value, isUpdated = false }) => {
  const renderCellContent = useMemo(() => {
    ...
    if (column.data_type.options) {
      const option = column.data_type.options.find((opt) => opt.value === value)
      if (option) {
        ...
        if (hasBgStyle(option.styles)) {
          return <Badge className={cn("inline-flex items-center gap-2 font-medium", option.styles)}>{content}</Badge>
        }

        // Si no hay estilos de fondo, usar div con los estilos
        return <div className={cn("inline-flex items-center gap-2", option.styles)}>{content}</div>
      }
    }

    // Formatear valor según el tipo
    let formattedValue: string = value.toString()
    if (column.data_type.primitive === "number" && column.data_type.format === "currency") {
    ...
    if (hasBgStyle(column.data_type.styles)) {
      return (
        <Badge className={cn("inline-flex items-center gap-2 font-medium", column.data_type.styles)}>
          {formattedValue}
        </Badge>
      )
    }
    ...
    return <span className={cn(column.data_type.styles)}>{formattedValue}</span>
  }, [column, value])

  return (
    <div
      className={cn("flex items-center", isUpdated && "bg-yellow-200 transition-colors duration-500")}
