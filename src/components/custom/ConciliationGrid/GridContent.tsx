...
import type { ConditionalRowFormat, Source } from "@/types/components/custom-table-conciliation-type"
...
const GridContentInner: React.FC<GridContentProps> = memo(
  ({
      ...
      return Object.entries(source.body.scheme)
        ...
        .map(([key, column]) => {
          return columnHelper.accessor(key, {
            header: () => (
              <div className="cursor-pointer select-none" onClick={() => onSort(key)}>
                {column.display}
                {sortState.column === key && <span className="ml-2">{sortState.direction === "asc" ? "▲" : "▼"}</span>}
              </div>
            ),
            cell: (info) => (
              <TableCell
    ...
    if (!source.body.scheme || (!isLoading && !source.body.datarows.length)) {
      return <div className="h-full flex items-center justify-center text-gray-500">No hay datos disponibles</div>
    }
    ...
    return (
      <div className="h-full flex flex-col">
        <div className={cn("bg-white border-b border-gray-200", source.behaviors.isSticky && "sticky top-0 z-10")}>
          <div className="px-4 py-1 font-medium text-sm border-b border-gray-200">{source.display}</div>
        </div>
        <div
          ref={parentRef}
          ...
          style={{
            height: `calc(100% - ${source.behaviors.isSticky ? "32px" : "0px"})`,
          }}
        >
          <table ref={tableRef} className="w-full border-separate border-spacing-0">
            ...
            <tbody>
              ...
              {virtualRows.map((virtualRow) => {
                ...
                const baseStyles = matchingFormat ? matchingFormat.styles.join(" ") : ""
                ...
                return (
                  <tr
                    ...
                    className={cn(baseStyles, "transition-all duration-700 hover:bg-gray-50 relative")}
                    role="row"
  ...
  return (
    <ErrorBoundary
      fallback={
        <div className="h-full flex flex-col">
          <div
            className={cn("bg-white border-b border-gray-200", props.source.behaviors.isSticky && "sticky top-0 z-10")}
          >
            <div className="px-4 py-1 font-medium text-sm border-b border-gray-200">{props.source.display}</div>
          </div>
