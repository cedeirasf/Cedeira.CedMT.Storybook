...
export const StatusBadge: React.FC<StatusBadgeProps> = ({ value, options = [], className }) => {
  const getStatusConfig = (display: string): { icon: React.ElementType | null; color: BadgeProps["color"] } => {
    ...
    if (displayLower.includes("ignorada") || displayLower.includes("no aplica")) {
  ...
  return (
    <Badge color={hasCustomStyles ? undefined : color} className={cn(hasCustomStyles && option.styles, className)}>
